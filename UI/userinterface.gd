...
@onready var infoText = get_node("CanvasLayer/Infotext")
...
func _info_text(text):
	infoText.text = str(text)
	infoText.modulate.a = 1
	
	var tween = infoText.create_tween()
	tween.set_ease(Tween.EASE_IN)
	tween.set_trans(Tween.TRANS_QUART)
	tween.tween_property(infoText, "modulate:a", 0, 5)
